...
cd ..
docker compose up --build --scale mysql=0
```

> On AWS the root compose doesn't need the local MySQL service. If you only need the frontend, the MySQL container will start but remain unused since backend connects directly to RDS. To avoid starting MySQL, you can scale it to zero: `docker compose up --build --scale mysql=0`
