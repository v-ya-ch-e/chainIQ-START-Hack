"""Convert any file into a structured purchase request JSON.

Accepts JSON (with optional gap-filling from request_text) or unstructured
files (converted via Anthropic API). Both paths converge through the same
schema-enforcement and field-filling pipeline.

Usage:
  python createRequest.py <input_file> [output.json]
  No args  — read from stdin, write to stdout
  1 arg    — read from file, write to stdout
  2 args   — read from file, write to output file
...
def main():
    ...
    if len(sys.argv) in (2, 3):
        ...
        if len(sys.argv) == 3:
            ...
            print(f"[{status}] Request created: {req.get('request_id', '(no ID)')}")
    ...
    print(
        f"Usage: {sys.argv[0]} [<input_file> [output.json]]\n"
        f"  No args  — read from stdin, write to stdout\n"
        f"  1 arg    — read from file, write to stdout\n"
        f"  2 args   — read from file, write to output file",
        file=sys.stderr,
    )
    sys.exit(1)
